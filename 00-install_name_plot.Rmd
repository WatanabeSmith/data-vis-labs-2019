...
Launch RStudio. You should get a window similar to the screenshot you see [here](https://www.rstudio.com/wp-content/uploads/2014/04/rstudio-workbench.png), but yours will be empty. Look at the bottom left pane: this is the same console window you saw when you opened R in step 2. 
...
```{r c_demo}
c("hello", "my", "name", "is", "kevin")
c(1:3, 20, 50)
```

Mind your use of quotes carefully with packages.

* To *install* a package, you put the name of the package in quotes as in `install.packages("name_of_package")`. You only need to do this once per package for each computer where you install R. 
...
You can execute all of the code in your R Script by clicking "Run" (or Ctrl + Alt + R), or you can run just the line your cursor is on by pressing "Ctrl + Enter".
...
```{r hlo_dataset}
glimpse(babynames) # dplyr function
head(babynames) # base R function
tail(babynames) # base R function
colnames(babynames) # base R function
```


If you have done the above and produced sane-looking output, you are ready for the next bit. Use the code below to create a new data frame called `sierra_alt`. My daughter Sierra was born last year, and I want to know if we spelled her name in a common or unique fashion.

```{r manipulate_name_data}
sierra_alt <- babynames %>% 
  filter(name == "Sierra" | name == "Siera" | name == "Ciara" | name == "Cierra" | name == "Seara") %>% 
  filter(sex == "F")

```

* The first bit makes a new dataset called `sierra_alt` that is a copy of the `babynames` dataset - the `%>%` tells you we are doing some other stuff to it later. While this is written first, the `<-` assignment or copying happens last.

* The second bit `filters` our `babynames` to only keep rows where the `name` is Sierra or an alternate spelling (read each `|` as _"or"_.) 

* The third bit applies another `filter` to keep only those where sex is female.

Let's check out the data.

```{r checking_manipulated_data}
sierra_alt
glimpse(sierra_alt)
```
...
```{r plot_name_data}
plot <- ggplot(sierra_alt, aes(x = year, 
                           y = prop,  
                           group = name, 
                           color = name)) + 
  geom_line()  
...
## Remixing name plots - a preview of what you'll learn

That last Sierra plot is hard to read which names are which lines (probably because of my color blindness), but we can add labels to each line with an extra line or two.

```{r}
ggplot(sierra_alt, aes(x = year,
                       y = prop,
                       group = name,
                       color = name)) +
  geom_line() + 
  geom_text(data = sierra_alt %>% filter(year == 2000), 
            aes(x = year, 
                y = prop, 
                label = name), 
            nudge_y = 0.0001)
...
Ciara became quite popular following the singer by the same name releasing a triple-platnum debut in 2004.

What about Star Wars characters, which names rose in popularity after the release of Star Wars?

```{r starwars1}
swnames <- babynames %>% 
  filter((name == "Han" & sex == "M") |
           (name == "Luke" & sex == "M") |
           (name == "Leia" & sex == "F"))

ggplot(swnames, aes(x = year,
                    y = prop,
                    group = name,
                    color = name)) +
  geom_line() +
  geom_vline(xintercept = 1977, linetype = "3133")
```

How about the new movies, is Rey usually a male or female name? Is that changing?

```{r starwars2}
rey_gender <- babynames %>% 
  filter(name == "Rey")

ggplot(rey_gender, aes(x = year,
                    y = prop,
                    group = sex,
                    color = sex)) +
  geom_line() +
  geom_vline(xintercept = 2015, linetype = "2123")
```

What about the course instructors? Can we guess how old they are by their names?

```{r instructornames}
instructors <- babynames %>% 
  filter((name == "Steven" & sex == "M") |
           (name == "Kevin" & sex == "M") |
           (name == "Jackie" & sex == "F"))

ggplot(instructors, aes(x = year,
                    y = prop,
                    color = name)) +
  geom_line()
```
...
```{r eval = F}
ggsave("kevin_watanabesmith.pdf", plot) # please make the filename unique!
```

**For Rstudio cloud**: once you save your plot as a PDF you'll still need to download the PDF to your computer. To do this, in the lower left panel click on the "Files" tab, check the box next to your saved PDF, click "More" drop-down menu and select "Export".
